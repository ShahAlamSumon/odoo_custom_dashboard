...
export class OwlPurchaseDashboard extends Component {
    // top products
    ...
    async getOrders(){
        ...
        //revenues
        const current_revenue = await this.orm.readGroup("purchase.order", domain, ["amount_total:sum"], [])
        const prev_revenue = await this.orm.readGroup("purchase.order", prev_domain, ["amount_total:sum"], [])
        const revenue_percentage = ((current_revenue[0].amount_total - prev_revenue[0].amount_total) / prev_revenue[0].amount_total) * 100

        //average
        const current_average = await this.orm.readGroup("purchase.order", domain, ["amount_total:avg"], [])
    ...
    async viewQuotations(){
        ...
        let list_view = await this.orm.searchRead("ir.model.data", [['name', '=', 'purchase_order_kpis_tree']], ['res_id'])
    ...
    async getTopProducts(){
        ...
        const data = await this.orm.readGroup("purchase.report", domain,
    ...
    async getMonthlySales(){
        ...
        const orders = data.filter(d => ['purchase','done'].includes(d.state))
        ...
        this.state.monthlySales = {
            data: {
                labels: labels,
                  ...
                  {
                    label: 'Quotations',
                    data: labels.map(l=>quotations.filter(q=>l==q.date_order).map(j=>j.price_total).reduce((a,c)=>a+c,0)),
                    hoverOffset: 4,
                    backgroundColor: "red",
                  },{
                    label: 'Orders',
                    data: labels.map(l=>orders.filter(q=>l==q.date_order).map(j=>j.price_total).reduce((a,c)=>a+c,0)),
                    hoverOffset: 4,
    ...
    async getPartnerOrders(){
        ...
        const data = await this.orm.readGroup("purchase.report", domain, ['partner_id', 'price_total', 'qty_ordered'], ['partner_id'], { orderby: "partner_id", lazy: false })
...
OwlPurchaseDashboard.template = "owl.OwlPurchaseDashboard"
OwlPurchaseDashboard.components = { KpiCard, ChartRenderer }

registry.category("actions").add("owl.purchase_dashboard", OwlPurchaseDashboard)